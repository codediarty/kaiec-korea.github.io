...
def build_members():
  ...
  (function(){
    ...
    /* 임원진 배지는 맨 앞 위원장 옆에 'KAIEC 임원진' 하나만 (2026.09.26 사용자: 그룹마다 '임원' 반복은 조잡) */
    var BADGE={'위원장':'KAIEC 임원진','전문위원':'KAIEC 소속 출강','AI 윤리 캠페인위원':'KAIEC 공식 위촉'};
    var NOTE={
      '전문위원':'KAIEC 소속 전문위원이 대학·기업·공공기관을 대상으로 맞춤형 AI 교육을 진행합니다. <a href="lecture.html#request">출강 문의 →</a>',
      'AI 윤리 캠페인위원':'위원회 검토를 거쳐 공식 위촉된 위원으로, 책임 있는 AI 문화 정착과 긍정적 사회가치 확산에 함께합니다.'};
    function grpHead(g){
    ...
    function secHTML(g,mode){
      ...
      if(mode==='lead'){
        ...
        return '<div class="lead-grp lg-'+n+'"><div class="grp-head">'+grpHead(g)+'</div><div class="member-grid">'+cards+'</div></div>';
      }
    ...
    var html='';
    /* 상단 (2026.09.26): 위원장 · 부위원장 · 고문·자문위원 / 사무국(사무총장·감사) · 전문위원 을 4열 격자 두 줄로.
       그룹마다 카드 수만큼 칸을 차지하므로 카드 크기는 아래 전문위원·캠페인위원과 같습니다. */
    var lead=['위원장','부위원장','고문·자문위원','사무국','전문위원','감사'].map(function(g){return secHTML(g,'lead')}).join('');
    ...
    if(named.length){
      var cards=named.map(function(m){
        return card({role:'AI 윤리 캠페인위원',name:m.name,en:m.en,code:m.code,email:m.email,since:m.since,field:m.field||'AI 윤리 캠페인 · 확산',photo:m.photo},'AI 윤리 캠페인위원');
      }).join('');
      ...
      html+='<div style="margin-bottom:44px">'
        +grpHead('AI 윤리 캠페인위원')
        +'<div class="member-grid camp-grid">'+cards+'</div></div>';
    }
    box.innerHTML=html||'<p style="text-align:center;color:var(--gray-500);padding:40px 0">위원 명단은 준비 중입니다.</p>';
    /* 디지털 명함 (2026.09.26 4차 · 공식 제출용): 실명 카드를 누르면 위원회 로고 · 사진 · 직위(자격 표기 cred 포함) · 영문 이름 · 인증 문구 ·
       ...
       (2026.09.26 밤 12차 사용자 제안, 14차부터 화면 명함의 QR 아래 글씨 없음), 코드가 없는 임원 · 전문위원은 과정 소개 kaiec.kr/expert-apply/.
    ...
    /* 위원회 공식 색 (사이트 배지 · 머리띠와 같은 값) */
    var INK='#0A1628',NAVY='#0F2A5F',NAVY2='#17325F',BLUE='#1F5FE0',BLUE6='#1749B4',STEEL='#9FB3D1',
        TXT='#0B1B36',G7='#374151',G6='#4B5563',G5='#6B7280',G4='#9CA3AF',LINE='#E3E8EF';
    var UNIT={'고문·자문위원':'고문·자문위원단','사무국':'사무국','전문위원':'전문위원회','AI 윤리 캠페인위원':'AI 윤리 캠페인위원단'};
    var SINCE={'위원장':'취임','부위원장':'선임','감사':'선임','사무국':'선임'};
    var CHIP={'전문위원':'KAIEC 소속 출강 강사','AI 윤리 캠페인위원':'한국AI윤리위원회 공식 위촉'};
    var CHIP_DEF='한국AI윤리위원회 공식 명단 등재';
    ...
    function roleLine(p){return (p.m.cred?p.m.cred+' · ':'')+titleOf(p)}
    function unitOf(p){
      var parts=(p.m.role||p.g).split(' · '),u=UNIT[p.g]||'';
      if(parts.length>1)u=(u?u+' ':'')+parts.slice(1).join(' · ');
      return u;
    }
    var TAGLINE='책임 있는 AI 활용과 올바른 AI 윤리문화 확산에 함께합니다.';
    function pid(p,i){
      if(p.m.code)return p.m.code;
    ...
    function qrLabel(m){return m&&m.code?'AI윤리전문가 과정 신청':'AI윤리전문가 과정 안내'}   /* 화면에는 글씨 없이 QR 만 (2026.09.26 밤 14차), 읽기 도구용 이름 */
    ...
    /* ── 명함 이미지 (canvas, 논리 1080×1350 × 1.6배) : 공식기관 디지털 명함 ─────────────────── */
    var SCALE=1.6,CW=1080,CH=1350;   /* 디지털 명함 논리 크기(4:5) × 1.6배 = 1728×2160 (휴대폰에서 빨리 만들어지도록) */
    function svgData(el){
    ...
    function wmURL(){
      ...
      for(var i=0;i<ps.length;i++){var id=ps[i].getAttribute('id');ps[i].removeAttribute('id');ps[i].setAttribute('style','fill:'+(id==='wm-en'?STEEL:'#FFFFFF'))}
    ...
    function wrapText(ctx,t,max){
      var ws=t.split(' '),out=[],cur='';
      ws.forEach(function(w){var nx=cur?cur+' '+w:w;if(!cur||ctx.measureText(nx).width<=max)cur=nx;else{out.push(cur);cur=w}});
    ...
    function paint(ctx,p,id,photo,badge,wm){
      /* 디지털 명함 (2026.09.26 밤 14차, 사용자: '위원증이라기보다 디지털 명함으로, 공식 위원증 문구만 우측 상단에 살짝,
         진짜 공식기관 느낌, 글씨가 작아 가독성이 떨어진다'). 휴대폰 화면 폭(약 390px)에서 읽히도록 글씨를 키우고 항목을 줄임.
         머리띠(배지 · 워드마크 | 공식 위원증) → 사진 + 직위 · 성명 · 영문 · 위원 코드 · 위촉 → 소속 · 분야 · 이메일 → QR 과 과정 안내 · 공식 명단 → 바닥띠 */
      var m=p.m,W=CW,H=CH,M=72,title=titleOf(p),unit=unitOf(p),lect=p.g==='전문위원',HB=176;
      ctx.textBaseline='alphabetic';ctx.textAlign='left';ctx.lineCap='butt';ctx.lineJoin='miter';
      ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);
      /* 머리띠: 위원회 배지와 같은 진남색 + 옅은 물결 선 */
      var g=ctx.createLinearGradient(0,0,W,HB);g.addColorStop(0,INK);g.addColorStop(.62,NAVY);g.addColorStop(1,NAVY2);
      ...
      for(var k=0;k<18;k++){ctx.beginPath();for(var x=0;x<=W;x+=6){var yy=-20+k*12+10*Math.sin(x/58+k*.62);if(x)ctx.lineTo(x,yy);else ctx.moveTo(x,yy)}ctx.stroke()}
      ctx.restore();
      ctx.fillStyle=BLUE;ctx.fillRect(0,HB,W,6);
      /* 로고(왼쪽): 진남색 배지(안쪽 실선) + 흰 워드마크 */
      var bw=136,bh=62,bx=M,by=(HB-bh)/2;
      var bgr=ctx.createLinearGradient(0,by,0,by+bh);bgr.addColorStop(0,'#1E3F75');bgr.addColorStop(1,'#10264D');
      ctx.fillStyle=bgr;rr(ctx,bx,by,bw,bh,11);ctx.fill();
      ctx.strokeStyle='rgba(255,255,255,.2)';ctx.lineWidth=1.5;rr(ctx,bx+.75,by+.75,bw-1.5,bh-1.5,10.25);ctx.stroke();
      ctx.strokeStyle='rgba(255,255,255,.3)';rr(ctx,bx+6,by+6,bw-12,bh-12,6);ctx.stroke();
      if(badge){var kw=88,kh=kw*10.66/59.04;ctx.drawImage(badge,bx+(bw-kw)/2,by+(bh-kh)/2,kw,kh)}
      else{ctx.fillStyle='#fff';ctx.font='800 23px '+LATIN;spaced(ctx,'KAIEC',bx+bw/2+2,by+bh/2+8,4,'center')}
      var wh=58,ww=wh*169.68/31.95,wx=bx+bw+22;
      if(wm){ctx.drawImage(wm,wx,by+(bh-wh)/2,ww,wh)}
      else{ctx.fillStyle='#fff';ctx.font='900 40px '+SERIF;ctx.fillText(ORG,wx,by+40);
        ctx.fillStyle=STEEL;ctx.font='700 13px '+LATIN;spaced(ctx,'KOREA AI ETHICS COMMITTEE',wx+2,by+60,3,'left')}
      /* 공식 위원증 (오른쪽 위, 작게) */
      ctx.textAlign='right';ctx.fillStyle='#fff';ctx.font='800 25px '+FONT;spaced(ctx,'공식 위원증',W-M,HB/2+2,2,'right');
      ctx.fillStyle=STEEL;ctx.font='700 11.5px '+LATIN;spaced(ctx,'OFFICIAL MEMBER CARD',W-M,HB/2+28,3.2,'right');ctx.textAlign='left';
      /* 사진 3:4 (흰 여백 + 옅은 테두리) */
      var fx=M,fy=HB+6+50,pw=264,ph=352,px=fx+8,py=fy+8;
      ctx.save();ctx.shadowColor='rgba(10,22,40,.10)';ctx.shadowBlur=14;ctx.shadowOffsetY=4;ctx.fillStyle='#fff';ctx.fillRect(fx,fy,pw+16,ph+16);ctx.restore();
      ctx.strokeStyle='#CBD5E1';ctx.lineWidth=1.5;ctx.strokeRect(fx+.75,fy+.75,pw+14.5,ph+14.5);
      ...
      if(photo){
        ...
        ctx.fillStyle=BLUE6;ctx.font='800 124px '+FONT;ctx.textAlign='center';
        ctx.fillText((m.name||'?').replace(/[^가-힣A-Za-z]/g,'').slice(0,1)||'·',px+pw/2,py+ph/2+44);ctx.textAlign='left';
      }
      ctx.restore();
      /* 오른쪽: 직위 · 성명 · 영문, 위원 코드(없으면 직위) · 위촉 */
      var rx=fx+pw+16+48,rw=W-M-rx,rl=roleLine(p),top=fy;
      ctx.fillStyle=BLUE6;fitFont(ctx,rl,rw,31,700);ctx.fillText(rl,rx,top+40);
      ctx.fillStyle=INK;fitFont(ctx,m.name,rw,90,800);ctx.fillText(m.name,rx,top+136);
      if(m.en){ctx.fillStyle=G5;ctx.font='700 22px '+LATIN;spaced(ctx,m.en.toUpperCase(),rx+2,top+180,4.5,'left')}
      ctx.fillStyle=LINE;ctx.fillRect(rx,top+216,rw,1.5);
      var lab=m.code?'위원 코드':'직위',lab2=m.code?'MEMBER CODE':'POSITION';
      ctx.fillStyle=G6;ctx.font='600 22px '+FONT;ctx.fillText(lab,rx,top+262);
      var lw=ctx.measureText(lab).width;ctx.fillStyle=G4;ctx.font='700 12px '+LATIN;spaced(ctx,lab2,rx+lw+12,top+261,3,'left');
      if(m.code){ctx.fillStyle=BLUE6;ctx.font='800 46px '+LATIN;spaced(ctx,m.code,rx-1,top+322,5,'left')}
      else{ctx.fillStyle=INK;fitFont(ctx,title,rw*.56,40,800);ctx.fillText(title,rx,top+320)}
      if(m.since){var tx=rx+Math.round(rw*.62),sl=sinceLabel(p);ctx.fillStyle=G6;ctx.font='600 22px '+FONT;ctx.fillText(sl,tx,top+262);
        var slw=ctx.measureText(sl).width;ctx.fillStyle=G4;ctx.font='700 12px '+LATIN;spaced(ctx,'SINCE',tx+slw+12,top+261,3,'left');
        ctx.fillStyle=INK;ctx.font='800 38px '+LATIN;ctx.fillText(m.since,tx,top+320)}
      /* 소속 · 분야 · (출강) · 이메일 */
      var rows=[['소속',ORG+(unit?' · '+unit:'')]];
      if(m.field)rows.push([FIELD[p.g]||'활동 분야',m.field]);
      if(lect)rows.push(['출강','기관 맞춤형 현장 강의']);
      rows.push([m.email?'이메일':'대표 메일',m.email||EMAIL,'latin']);
      /* 세로 간격 고르게: 사진 아래 · 표 아래 · QR 아래(바닥띠 위) 세 틈을 같게 */
      var fb=H-148,qs=188,bt=fy+ph+16,rh=rows.length<=3?70:64,th=rows.length*rh+3.5,
          gp=Math.max(30,Math.min(90,Math.floor((fb-bt-th-qs)/3))),ty=bt+gp,vx=M+176,vw=W-M-vx;
      if(ty+th+gp+qs>fb-24){rh=Math.floor((fb-24-qs-gp-ty-3.5)/rows.length);th=rows.length*rh+3.5}
      ctx.fillStyle=INK;ctx.fillRect(M,ty,W-2*M,2);
      rows.forEach(function(r,i){
        var yb=ty+2+i*rh,base=yb+rh/2+11;
        ctx.fillStyle=G6;ctx.font='600 25px '+FONT;ctx.fillText(r[0],M+4,base);
        ctx.fillStyle=TXT;fitFont(ctx,r[1],vw,30,700,r[2]==='latin'?LATIN:FONT);ctx.fillText(r[1],vx,base);
        ctx.fillStyle=i<rows.length-1?LINE:INK;ctx.fillRect(M,yb+rh,W-2*M,i<rows.length-1?1:1.5);
      });
      /* QR 과 과정 안내 · 공식 명단 (QR: 위원 코드가 있으면 주문서 + 위원 코드 kaiec.kr/go/?c=코드, 없으면 과정 소개 kaiec.kr/expert-apply/) */
      var qx=M,qy=ty+th+gp;
      ...
      drawQR(ctx,qrURL(m),qx+14,qy+14,qs-28);
      var tx2=qx+qs+36,tw=W-M-tx2,qh=m.code?'AI윤리전문가(AIEP) 과정 신청':'AI윤리전문가(AIEP) 과정 안내';
      ctx.fillStyle=INK;fitFont(ctx,qh,tw,31,800);ctx.fillText(qh,tx2,qy+50);
      ctx.fillStyle=G5;fitFont(ctx,'QR 코드를 휴대폰 카메라로 스캔해 주세요',tw,21,600);ctx.fillText('QR 코드를 휴대폰 카메라로 스캔해 주세요',tx2,qy+86);
      [['과정 안내','kaiec.kr/experts'],['공식 명단',cardURL(id).replace(/^https?:\/\//,'')]].forEach(function(l,i){
        var yl=qy+136+i*40;
        ctx.fillStyle=G6;ctx.font='600 22px '+FONT;ctx.fillText(l[0],tx2,yl);
        ctx.fillStyle=BLUE6;fitFont(ctx,l[1],tw-112,23,700,LATIN);ctx.fillText(l[1],tx2+112,yl);
      });
      /* 바닥띠: 한 줄 다짐 + 연락처 */
      ...
      ctx.textAlign='center';ctx.fillStyle='#fff';fitFont(ctx,TAGLINE,W-2*M,27,700);ctx.fillText(TAGLINE,W/2,fb+62);
      ctx.fillStyle=STEEL;ctx.font='600 21px '+FONT;ctx.fillText(ORG+'   ·   kaiec.kr   ·   '+EMAIL,W/2,fb+104);ctx.textAlign='left';
    }
    function makeImage(p,id,photoSrc){
      var m=p.m,ko=[m.name,roleLine(p),unitOf(p),ORG,m.field||'',TAGLINE,
        ...
        'AI윤리전문가(AIEP) 과정 신청 안내 QR 코드를 휴대폰 카메라로 스캔해 주세요 대표 메일 0123456789.·'].join(' ');
    ...
    function saveImage(btn){
      ...
      var ios=isTouch()&&isIOS()&&!inApp();
      function restore(){if(btn._label){clearTimeout(btn._t);btn.innerHTML=btn._label}}
      /* 아이폰 공유 시트는 누르는 순간에만 열 수 있어, 이미지가 아직 준비 중이었다면 한 번 더 누르게 합니다(길게 눌러 저장 창으로 넘기지 않음) */
      function arm(){if(!btn._label)btn._label=btn.innerHTML;clearTimeout(btn._t);btn.innerHTML=ICON_OK+'준비 완료 · 한 번 더 눌러 저장'}
      function iosShare(blob){
        var file=null;try{file=new File([blob],name,{type:'image/png'})}catch(e){}
        if(!(file&&navigator.canShare&&navigator.canShare({files:[file]}))){showSave(blob,name);return}
        restore();
        navigator.share({files:[file]}).catch(function(err){
          if(err&&err.name==='AbortError')return;
          if(err&&err.name==='NotAllowedError'){arm();return}
          showSave(blob,name);
        });
      }
      function go(blob,now){
        ...
        if(ios){if(now)iosShare(blob);else arm();return}
        download(blob,name);flash(btn,ICON_OK+'저장됨 (다운로드 · 갤러리)');
      }
      if(rec.blob)go(rec.blob,true);
      else{flash(btn,'이미지 준비 중…');(rec.promise||Promise.resolve(null)).then(function(b){go(b,false)})}
    }
    ...
    function closeP(){
      ...
      if(curId&&history.replaceState)history.replaceState(null,'',location.pathname+location.search);
      curId='';curP=null;
    ...
    function openP(i,quiet){
      ...
      if(!ov){
        ...
        document.addEventListener('keydown',function(e){
          ...
          if(e.key==='Tab'){
            var f=ov.querySelectorAll('button,a[href]'),first=f[0],last=f[f.length-1];
      ...
      var idBox='<div class="bc-id"><div class="bc-id-main">'
        ...
        +'</div>'+(qr?'<div class="bc-qr-wrap"><div class="bc-qr" role="img" aria-label="'+qrLabel(m)+' QR 코드">'+qr+'</div></div>':'')+'</div>';
      var rows='<div><dt>소속</dt><dd>'+ORG+(unit?'<span class="bc-unit">'+unit+'</span>':'')+'</dd></div>'
        ...
        +(m.email?'<div><dt>이메일</dt><dd><a href="mailto:'+m.email+'">'+m.email+'</a></dd></div>'
      ...
      ov.querySelector('.bc-body').innerHTML=
        ...
        +bigAvatar(m)
        +'<div class="bc-role">'+roleLine(p)+'</div>'
    ...
    /* #PKH3185 · #shin-dong-bok 처럼 들어오면 해당 명함을 바로 엽니다 */
    (function(){
...
def build_expert_apply():
    ...
    GOAL = ' class="is-goal"'
    STEPS_HTML = "".join(
        f'<li{GOAL if i == len(STEPS) - 1 else ""}><span class="ea-step-ic"><i data-lucide="{ic}"></i></span>'
        f'<b>{t}</b><small>{d}</small></li>' for i, (ic, t, d) in enumerate(STEPS))
    GETS = [
        ("award", "공식 이수증", "고유 이수번호 기재"),
        ("badge-check", "홈페이지 전문가 등록", "이름·이수번호로 확인"),
        ("file-text", "이력서 활용 가이드", "표기법·자기소개서 예시"),
        ("user-plus", "전문위원 신청 자격", "강사·자문 활동으로 확장"),
    ]
    GETS_HTML = "".join(
