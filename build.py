...
def build_members():
  ...
  (function(){
    ...
    /* 임원진(위원장·부위원장·고문·자문위원·사무국·감사)은 배지 없이 직함만 (2026.09.26 사용자: '임원' 반복이 조잡) */
    var BADGE={'전문위원':'KAIEC 공식 출강','AI 윤리 캠페인위원':'KAIEC 공식 위촉'};
    var NOTE={
    ...
    var html='';
    /* 상단 임원진 (2026.09.26): 위원장 · 부위원장 · 고문·자문위원 / 사무국 · 감사 를 4열 격자 두 줄로.
       그룹마다 카드 수만큼 칸을 차지하므로 카드 크기는 아래 전문위원·캠페인위원과 같습니다. */
    var lead=['위원장','부위원장','고문·자문위원','사무국','감사'].map(function(g){return secHTML(g,'lead')}).join('');
    ...
    if(named.length){
      var cards=named.map(function(m){
        return card({role:'AI 윤리 캠페인위원',name:m.name,en:m.en,code:m.code,since:m.since,field:m.field||'AI 윤리 캠페인 · 확산',photo:m.photo},'AI 윤리 캠페인위원');
      }).join('');
    ...
    var CHIP={'전문위원':'KAIEC 공식 출강 강사','AI 윤리 캠페인위원':'한국AI윤리위원회 공식 위촉'};
    ...
    function statementOf(p){
      var pre={'AI 윤리 캠페인위원':'공식 위촉 ','전문위원':unitOf(p)+' ','사무국':'사무국 '}[p.g]||'';
      return '위 사람은 '+ORG+' '+pre+titleOf(p)+'임을 확인합니다.';
    }
    function pid(p,i){
    ...
    function paint(ctx,p,id,photo,badge,wm){
      ...
      ctx.textAlign='center';ctx.fillStyle=G7;ctx.font='700 16px '+FONT;ctx.fillText('AI윤리전문가 양성과정 신청',qx+qs/2,qy+qs+32);
      ctx.fillStyle=G4;ctx.font='700 11px '+LATIN;spaced(ctx,'SCAN TO APPLY',qx+qs/2,qy+qs+54,3,'center');ctx.textAlign='left';
      /* 소속 · 분야 · (출강) · 발급일 */
      var y=784,vx=L+150,vmax=qx-40-vx;
      function row(label,val,sub){
        ctx.fillStyle=G5;ctx.font='600 21px '+FONT;ctx.fillText(label,L,y);
        ctx.fillStyle=TXT;fitFont(ctx,val,vmax,26,700);ctx.fillText(val,vx,y);
        if(sub){ctx.fillStyle=G6;fitFont(ctx,sub,vmax,21,600);ctx.fillText(sub,vx,y+32);y+=32}
        y+=58;
      }
      row('소속',ORG,unit);
      if(m.field)row(FIELD[p.g]||'활동 분야',m.field);
      if(lect)row('출강','기관 맞춤형 현장 강의');
      row('발급일',today());
      /* 확인 문구 + 명단 확인 주소 */
      var sy=1040,sh=156;
      ctx.fillStyle='#F4F6FA';rr(ctx,L,sy,R-L,sh,16);ctx.fill();
      ctx.fillStyle=G7;ctx.font='600 24px '+FONT;
      var lines=wrapText(ctx,statementOf(p),R-L-80).slice(0,2),ly=lines.length>1?sy+54:sy+64;
      lines.forEach(function(t,i){ctx.fillText(t,L+40,ly+i*36)});
      var vy=lines.length>1?sy+130:sy+112,vt='공식 명단 확인';
      ctx.fillStyle=G5;ctx.font='600 19px '+FONT;ctx.fillText(vt,L+40,vy);
      var vw=ctx.measureText(vt).width;ctx.fillStyle=BLUE6;ctx.font='700 19px '+LATIN;ctx.fillText(cardURL(id).replace(/^https?:\/\//,''),L+40+vw+14,vy);
      /* 바닥띠 */
    ...
    function makeImage(p,id,photoSrc){
      var m=p.m,ko=[m.name,roleLine(p),CHIP[p.g]||CHIP_DEF,unitOf(p),ORG,m.field||'',statementOf(p),
        '공식 위원증 소속 분야 담당 자문 전문 활동 출강 기관 맞춤형 현장 강의 발급일 위원 코드 직위 취임 선임 위촉 명단 확인 AI윤리전문가 양성과정 신청 0123456789.'].join(' ');
      var lat='KAIEC OFFICIAL MEMBER CARD SCAN TO APPLY KOREA AI ETHICS COMMITTEE MEMBER CODE POSITION SINCE kaiec.kr/members/# 0123456789.-· '+(m.en||'').toUpperCase()+(m.code||'')+EMAIL+id;
      var loads=[];
      if(document.fonts&&document.fonts.load){
    ...
    function openP(i,quiet){
      ...
      ov.querySelector('.bc-body').innerHTML=
        ...
        +'<div class="bc-cert"><p>'+statementOf(p)+'</p><span>'+today()+' · '+ORG+'</span></div>'
